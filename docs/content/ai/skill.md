...
Drop it into your agent's skill directory (or copy its triggers/rules into your own system) and the agent will:
...
```bash
# from inside doma's checkout, copy into your agent's skill dir:
...
The path above is for Claude Code; substitute your agent's equivalent location (e.g. `~/.codex/skills/`, project-local `.skills/`). Either way, the agent will load the skill on next session start.
